...
result_files = [
    f
    for f in listdir(join(results_path / "data"))
    if isfile(join(results_path / "data", f)) and f.endswith("otp.parquet")
]

pattern = re.compile(r"_(\d{2}_\d{2})_")
...
arrival_times = sorted(set(values))
    ...
    for service in services:
        ...
        for i in range(1, int(service["n_neighbors"]) + 1):
            ...
            arrival_deadline = pd.to_datetime(
                f"{config_model['travel_date']} {arrival_time.replace('_', ':')}"
            )
